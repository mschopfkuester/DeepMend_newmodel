...
class Decoder(nn.Module):
    def __init__(
        ...
        dims = [latent_size + 2*fourier_feature] + dims + [1]
        
        
        for k in latent_in:    ######adapt vector length for skip connection
    	    dims[k]= dims[k]+latent_size+fourier_feature*2   
        ...
        self.subnet_latent_in = subnet_latent_in

        self.subnet_latenet_in=subnet_latent_in
        ...
        self.register_buffer('h_B',torch.randn((fourier_feature,3)))
        self.register_buffer('f_B',torch.randn((fourier_feature,3)))

        # Build hnet
        #hnet_dims = [tool_latent_size + subnet_pts_in] + subnet_dims + [1]
        hnet_dims = [tool_latent_size + 2*fourier_feature] + subnet_dims + [1]

        for k in self.subnet_latent_in:    ######adapt vector length for skip connection
    	    hnet_dims[k]= hnet_dims[k]+tool_latent_size+fourier_feature*2   
    ...
    def forward(self, net_input, use_net=None):
        ...
        pts_fou_h = (self.f_B@pts.unsqueeze(-1))[...,0]
        #print(self.f_B[0])
        pts_fou_h = 2.0*torch.pi*pts_fou_h
        pts_fou_h = pts_fou_h.repeat_interleave(2, dim=-1)
        pts_fou_h[:,1::2] += 0.5*torch.pi
        pts_fou_h = torch.sin(pts_fou_h)
        
        pts_fou_f = (self.f_B@pts.unsqueeze(-1))[...,0]
        pts_fou_f = 2.0*torch.pi*pts_fou_f
        pts_fou_f = pts_fou_f.repeat_interleave(2, dim=-1)
        pts_fou_f[:,1::2] += 0.5*torch.pi
        pts_fou_f = torch.sin(pts_fou_f)

        h_input = torch.cat([break_latent_vecs, pts_fou_h], dim=1)
        f_input = torch.cat([latent_vecs, pts_fou_f], dim=1)
        ...
        def ff_hnet(x):
            ...
            for layer in range(0, self.hnet_num_layers - 1):
                # Feed forward
                if layer in self.subnet_latent_in:
